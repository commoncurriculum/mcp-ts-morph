...
`LOG_OUTPUT=console` かつ開発環境（`NODE_ENV !== 'production'`）で `pino-pretty` がインストールされている場合は、見やすい形式で出力されます。ログおよび起動時の診断メッセージはすべて標準エラー出力（stderr）に書き出されるため、MCP クライアントとの標準出力（JSON-RPC）を汚染しません。`LOG_LEVEL=silent` を指定すればログ出力を完全に抑止できます。
