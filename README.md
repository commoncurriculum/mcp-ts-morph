...
- **Reference updates**: `import Foo from "target"` → `import { Name } from "target"` (or `{ Name as Foo }` when the local name differs); default imports are merged into existing named imports, or split into a separate declaration when a namespace import (`import Foo, * as ns`) is present; `export { default } from "target"` and `export { default as X } from "target"` are rewritten to named re-exports. Path-alias and relative specifiers are both resolved via the TypeChecker.
